# algeria_training.py - Multi-task Learning for Segmentation and Geodesic Distance Prediction
...
# --- CONFIGURATION ---
# Directories for your processed Algeria data
PROCESSED_DATA_ROOT = "PROCESSED_DATA_FOR_SEGMENTATION"
ELEVEN_CHANNEL_DIR = os.path.join(PROCESSED_DATA_ROOT, "11CHANNEL_INPUTS") # Corrected: "11CHANNEL_INPUTS" instead of "11_CHANNEL_INPUTS"

# Segmentation Masks (0: Background, 1: Blade, 2: Vein)
COMBINED_MASKS_DIR = os.path.join(PROCESSED_DATA_ROOT, "GROUND_TRUTH_MASKS")
os.makedirs(COMBINED_MASKS_DIR, exist_ok=True)

# Geodesic Distance Masks (0-1 float for veins, 0 for non-veins)
GEODESIC_MASKS_DIR = os.path.join(PROCESSED_DATA_ROOT, "GEODESIC_MASKS")
os.makedirs(GEODESIC_MASKS_DIR, exist_ok=True) # Ensure this exists, though Stage 2 creates it
...
PRETRAINED_MODEL_PATH = "V1_best_model_vein_dice_0.7711_epoch16.pt" # Example path
...
LEARNING_RATE = 1e-4 # Adjusted learning rate as per previous successful run for Algeria
NUM_EPOCHS = 50
...
# NEW: Use half CPU cores for DataLoader, safer for multi-threading
NUM_WORKERS = 2
#NUM_WORKERS = os.cpu_count() // 2 if os.cpu_count() else 0

# Path to the file listing bad FIDS (relative to the script's current working directory)
# You can update this path if you prefer to store it elsewhere, e.g., in PROCESSED_DATA_ROOT
BAD_FIDS_FILE = "bad_fids.txt" # This will look for 'bad_fids.txt' in the same directory as the script
...
CLASS_WEIGHTS = torch.tensor([0.1, 1.0, 5.0], dtype=torch.float32) # Higher weight for Vein
...
TRAINING_CONFIG = {
    ...
    "CLASS_LABELS": CLASS_LABELS, # Stored as dict, not converted
    ...
    "NUM_WORKERS": NUM_WORKERS, # Add NUM_WORKERS to config
    "BAD_FIDS_FILE": BAD_FIDS_FILE # Add bad fids file path to config
}
...
class MultiChannelLeafDataset(Dataset):
    def __init__(self, eleven_channel_dir, blade_masks_dir, vein_masks_dir, combined_masks_output_dir, geodesic_masks_dir, transform=None, bad_fids_file=None): # ADDED bad_fids_file parameter
        ...
        # Original way to get all FIDS
        all_fids = [f.replace(".npy", "") for f in os.listdir(eleven_channel_dir) if f.endswith(".npy")]
        
        # NEW: Filter out bad FIDS
        self.bad_fids = set()
        if bad_fids_file and os.path.exists(bad_fids_file):
            with open(bad_fids_file, 'r') as f:
                for line in f:
                    self.bad_fids.add(line.strip()) # Add each FID from the file, remove whitespace
            print(f"Loaded {len(self.bad_fids)} bad FIDS from {bad_fids_file}.")
        else:
            print(f"No bad FIDS file found at {bad_fids_file}. All detected images will be processed.")
        
        # Filter fids
        self.fids = [fid for fid in all_fids if fid not in self.bad_fids]
        
        if not self.fids:
            raise RuntimeError(f"No valid .npy files found in {eleven_channel_dir} after filtering. Please ensure preprocessing Stage 2 completed and check your 'bad_fids' list.")

        # Pre-generate combined segmentation masks if they don't exist
        # This part should ideally also skip bad FIDS, but since it's already generated, it's fine.
        # If you were to re-run generation, you'd integrate bad_fids check here too.
    ...
    def _generate_combined_segmentation_masks(self):
        ...
        # Only generate for FIDS that are NOT in the bad_fids list
        for fid in tqdm(self.fids, desc="Generating Combined Segmentation Masks"):
            ...
            # THIS IS WHERE THE PATHS WERE LIKELY INCORRECT FOR YOUR SETUP
            # Based on your folder structure, it's highly probable the individual
            # blade and vein masks are directly under PROCESSED_DATA_ROOT.
            # Assuming they are named like 'FID_blade_mask.npy' and 'FID_vein_mask.npy'
            # and placed directly in 'PROCESSED_DATA_FOR_SEGMENTATION'
            # OR, if they are within a 'GROUND_TRUTH_MASKS' folder, we need to adjust.

            # Given your folder list, the most common setup is that
            # blade and vein masks are *sibling* directories to 11CHANNEL_INPUTS.
            # Since you list 'BLADE_COORDS' and 'VEIN_COORDS', those might be the *source*
            # of the masks, but not the masks themselves in .npy format.
            # Let's assume there are folders like 'BLADE_MASKS' and 'VEIN_MASKS'
            # (or the files are directly in PROCESSED_DATA_ROOT if they don't have dedicated folders).

            # From your previous script, you had:
            # blade_masks_dir=os.path.join(PROCESSED_DATA_ROOT, "BLADE_MASKS"),
            # vein_masks_dir=os.path.join(PROCESSED_DATA_ROOT, "VEIN_MASKS"),
            # However, your current directory listing does *not* show BLADE_MASKS or VEIN_MASKS.
            # It shows BLADE_COORDS and VEIN_COORDS, which are usually intermediate.
            # And then it shows GROUND_TRUTH_MASKS which *could* contain the individual masks.

            # Let's assume the .npy masks are inside 'GROUND_TRUTH_MASKS'
            # This is a common pattern:
            # PROCESSED_DATA_FOR_SEGMENTATION/GROUND_TRUTH_MASKS/FID_blade_mask.npy
            # PROCESSED_DATA_FOR_SEGMENTATION/GROUND_TRUTH_MASKS/FID_vein_mask.npy

            # Let's modify the paths for blade and vein masks within the dataset:
            ...
            if not os.path.exists(blade_mask_npy_path) or not os.path.exists(vein_mask_npy_path):
                print(f"WARNING: Missing blade or vein mask for {fid} at '{blade_mask_npy_path}' or '{vein_mask_npy_path}'. Cannot generate combined segmentation mask.")
                # If these are missing, it means the preprocessing to generate them was not correct
                # or they are in a different directory. For now, let's skip this FID.
                # You might want to add this FID to a 'bad_fids' list dynamically if this is a recurring issue.
                continue
...
def train_model(pretrained_model_path=None):
    ...
    full_dataset = MultiChannelLeafDataset(
        eleven_channel_dir=ELEVEN_CHANNEL_DIR,
        # *** IMPORTANT FIX HERE ***
        # blade_masks_dir and vein_masks_dir should point to where your individual .npy masks are.
        # Based on your folder structure, 'GROUND_TRUTH_MASKS' is the most probable location
        # if individual blade_mask.npy and vein_mask.npy files are within it.
        # If they are separate folders (e.g., BLADE_MASKS, VEIN_MASKS) that you didn't list,
        # then you'd need to add those.
        # For now, let's assume they are directly in PROCESSED_DATA_ROOT, or in a subfolder like 'GROUND_TRUTH_MASKS'
        # The original script assumed 'BLADE_MASKS' and 'VEIN_MASKS' as subfolders of PROCESSED_DATA_ROOT.
        # Since your listing shows 'GROUND_TRUTH_MASKS', let's assume they are there
        # but named like 'FID_blade_mask.npy' and 'FID_vein_mask.npy' inside.
        # However, the error suggests your `MultiChannelLeafDataset` (specifically `_generate_combined_segmentation_masks`)
        # is looking for `FID_blade_mask.npy` and `FID_vein_mask.npy` in the paths specified by `blade_masks_dir` and `vein_masks_dir`.
        # The safest bet based on your current listing of folders is that these are actually directly in PROCESSED_DATA_ROOT,
        # or that 'BLADE_MASKS' and 'VEIN_MASKS' existed before and are now gone, or they are within 'GROUND_TRUTH_MASKS'.
        # Let's try pointing them to 'GROUND_TRUTH_MASKS' as it's the most logical place for individual ground truth masks.
        blade_masks_dir=os.path.join(PROCESSED_DATA_ROOT, "GROUND_TRUTH_MASKS"), # <-- POTENTIAL FIX 1
        vein_masks_dir=os.path.join(PROCESSED_DATA_ROOT, "GROUND_TRUTH_MASKS"), # <-- POTENTIAL FIX 2
        combined_masks_output_dir=COMBINED_MASKS_DIR,
        geodesic_masks_dir=GEODESIC_MASKS_DIR,
        transform=DataAugmentation(),
        bad_fids_file=BAD_FIDS_FILE
    )
    print(f"Found {len(full_dataset)} valid Vinifera images after filtering.") # Changed message
    ...
    if pretrained_model_path and os.path.exists(pretrained_model_path):
        ...
        # Load state_dict, but account for potential new layers (geo_out_conv)
        # If the pre-trained model was only for segmentation, geo_out_conv won't be in state_dict.
        # Strict=False allows loading even if some keys are missing or unmatched (new layers added).
    ...
    # 3. Define Loss Functions, Optimizer, and Scheduler
    seg_criterion = nn.CrossEntropyLoss(weight=CLASS_WEIGHTS.to(DEVICE))
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        # We monitor the validation vein Dice score for improvement
        scheduler.step(val_dice_scores[2]) # Index 2 for 'Vein'

        # Print detailed scores
        print(f"\n--- Epoch {epoch}/{NUM_EPOCHS} ---")
        print(f"Time elapsed: {time.time() - start_time:.2f} seconds")
        print(f"Current Learning Rate: {optimizer.param_groups[0]['lr']:.8f}") # Print current LR
...
if __name__ == "__main__":
    ...
    os.makedirs(COMBINED_MASKS_DIR, exist_ok=True)
    # Ensure GEODESIC_MASKS_DIR exists (now pointing to the correct one)
    os.makedirs(GEODESIC_MASKS_DIR, exist_ok=True) 

    history = train_model(pretrained_model_path=PRETRAINED_MODEL_PATH)

    # Plot Training History
    plt.figure(figsize=(18, 12)) # Increased figure size for more plots and rows
    ...
    # Plot Dice Scores for all classes
    ...
    plt.plot(history['train_dice_blade'], label='Train Blade Dice', linestyle='--')
    plt.plot(history['val_dice_blade'], label='Val Blade Dice', linestyle='-')
    plt.plot(history['train_dice_vein'], label='Train Vein Dice', linestyle='--', linewidth=2)
    plt.plot(history['val_dice_vein'], label='Val Vein Dice', linestyle='-', linewidth=2)
    ...
    plt.plot(history['train_geodesic_mae'], label='Train Geodesic MAE')
    plt.plot(history['val_geodesic_mae'], label='Validation Geodesic MAE')
    ...
    plt.savefig(os.path.join(CHECKPOINTS_DIR, "training_history_plots.png"))
    print(f"Training history plots saved to: {os.path.join(CHECKPOINTS_DIR, 'training_history_plots.png')}")
    plt.show()