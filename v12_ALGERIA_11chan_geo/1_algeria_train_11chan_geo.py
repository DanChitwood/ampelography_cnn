...
PROCESSED_DATA_ROOT = "PROCESSED_DATA_FOR_SEGMENTATION_ALGERIA"
...
# PRETRAINED_MODEL_PATH = "VINIFERA4_best_model_vein_dice_0.6806_epoch21.pt" # Example path
...
class MultiChannelLeafDataset(Dataset):
    def __init__(self, eleven_channel_dir, blade_masks_dir, vein_masks_dir, combined_masks_output_dir, geodesic_masks_dir, transform=None, bad_fids_file=None): # ADDED bad_fids_file parameter
        ...
        self.bad_fids = set() # Corrected: Removed invisible character before 'self'
...
def train_model(pretrained_model_path=None):
    ...
    full_dataset = MultiChannelLeafDataset(
        ...
        geodesic_masks_dir=GEODESIC_MASKS_DIR, # NOW POINTS TO THE CORRECT FOLDER
        transform=DataAugmentation(),
        bad_fids_file=BAD_FIDS_FILE # PASS THE BAD FIDS FILE HERE
    )
...
if __name__ == "__main__":
    ...
    plt.figure(figsize=(18, 8)) # Increased figure size for more plots
    ...
    plt.plot(history['val_dice_vein'], label='Val Vein Dice', linewidth=2, color='red') # Continue plotting here
    ...
    # Plot Geodesic MAE
    plt.figure(figsize=(10, 5)) # New figure for MAE
    ...
    plt.title('Geodesic Mean Absolute Error over Epochs')
    plt.xlabel('Epoch')
    plt.ylabel('MAE')
    plt.legend()
    plt.grid(True)
