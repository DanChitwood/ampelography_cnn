# generate_ground_truth_masks.py
...
for fname in tqdm(image_files, desc="Creating masks"):
    ...
    if os.path.exists(mask_output_path):
        # tqdm doesn't show continue messages well, so only print if not in tqdm
        # print(f"  Mask for {base_name}.png already exists, skipping.")
        continue
