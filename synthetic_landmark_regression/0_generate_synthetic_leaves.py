...
# --- Configuration Parameters (from previous user code) ---
BOUND_RADIUS = 1
...
SAVED_MODEL_DIR = Path("saved_leaf_model_data")
...
# Grayscale values for output mask file (updated based on user request)
...
def process_synthetic_leaf(
    ...
    """
    Processes a single synthetic leaf's flattened coordinates to produce masks, ECTs, and viz.
    """
    current_metadata = {
        ...
        "file_combined_viz_vein_on_ect": "",    # Vein mask on vein ECT (updated)
        # --- NEW: Add placeholder for base/tip coordinates ---
        "base_x": np.nan,
        "base_y": np.nan,
        "tip_x": np.nan,
        "tip_y": np.nan
    ...
    ect_result_blade = None # <--- ADDED THIS LINE TO INITIALIZE
    ...
    try:
        ...
        # --- END Apply Random Rotation ---

        # --- Calculate Raw Base and Tip Points (on rotated_raw_vein_coords) ---
        # Base: Average of first and last vein coordinates
        # Ensure raw_vein_coords has at least 2 points for 0 and -1 index
        if len(rotated_raw_vein_coords) >= 2:
            raw_base_point = (rotated_raw_vein_coords[0] + rotated_raw_vein_coords[-1]) / 2.0
        elif len(rotated_raw_vein_coords) == 1: # If only one point, use it as base
            raw_base_point = rotated_raw_vein_coords[0]
        else:
            raise ValueError(f"Not enough vein coordinates ({len(rotated_raw_vein_coords)}) to calculate base point.")

        # Tip: Index position 608 (half of 1216)
        if len(rotated_raw_vein_coords) > NUM_VEIN_COORDS // 2:
            raw_tip_point = rotated_raw_vein_coords[NUM_VEIN_COORDS // 2]
        else:
            # Fallback if there are fewer than expected vein coords, pick the last one
            raw_tip_point = rotated_raw_vein_coords[-1] if len(rotated_raw_vein_coords) > 0 else np.array([np.nan, np.nan])

        # --- END Calculate Raw Base and Tip ---


        # --- Validate Blade Coordinates for ECT ---
        if len(np.unique(rotated_raw_blade_coords, axis=0)) < 3:
            raise ValueError(f"Synthetic leaf '{synthetic_id}' has too few distinct blade points ({len(np.unique(rotated_raw_blade_coords, axis=0))}) for ECT calculation.")
        ...
        # --- NEW: Transform Base and Tip Points into Blade's ECT Space ---
        # Apply the same affine transformation that was applied to the blade and veins
        transformed_base_point = apply_transformation_with_affine_matrix(raw_base_point.reshape(1, -1), ect_affine_matrix_blade).flatten()
        transformed_tip_point = apply_transformation_with_affine_matrix(raw_tip_point.reshape(1, -1), ect_affine_matrix_blade).flatten()

        current_metadata["base_x"] = transformed_base_point[0]
        current_metadata["base_y"] = transformed_base_point[1]
        current_metadata["tip_x"] = transformed_tip_point[0]
        current_metadata["tip_y"] = transformed_tip_point[1]
        # --- END NEW ---

        ect_result_blade = ect_calculator.calculate(G_blade) # Moved assignment here
        ...
        transformed_vein_pixels_for_vein_ect_viz = np.array([])
    ...
    for i in range(total_synthetic_samples):
        ...
        if len(flat_coords) != FLATTENED_COORD_DIM:
            ...
            metadata_records.append({
                ...
                "base_x": np.nan, "base_y": np.nan, "tip_x": np.nan, "tip_y": np.nan # Ensure these are present even if skipped
            })
