# 4_train_vinifera_model.py - Multi-task Learning for Segmentation and Geodesic Distance Prediction
...
# --- CONFIGURATION ---
# Base directory for all processed data.
# This MUST point to the 'processed_synthetic_data' directory created by 3_generate_11channel_inputs.py
PROCESSED_DATA_ROOT = "processed_synthetic_data" # <<-- MODIFIED HERE

# Directories for your processed data
ELEVEN_CHANNEL_DIR = os.path.join(PROCESSED_DATA_ROOT, "11_CHANNEL_INPUTS")
# COMBINED_MASKS_DIR is where the dataset will SAVE the combined segmentation masks
# (Blade=1, Vein=2, Background=0) from the individual blade and vein mask .npy files.
COMBINED_MASKS_DIR = os.path.join(PROCESSED_DATA_ROOT, "SEGMENTATION_MASKS_COMBINED") # <<-- MODIFIED HERE (renamed for clarity)
BLADE_MASKS_INPUT_DIR = os.path.join(PROCESSED_DATA_ROOT, "BLADE_MASKS") # <<-- NEW INPUT DIR
VEIN_MASKS_INPUT_DIR = os.path.join(PROCESSED_DATA_ROOT, "VEIN_MASKS") # <<-- NEW INPUT DIR
GEODESIC_MASKS_DIR = os.path.join(PROCESSED_DATA_ROOT, "GEODESIC_MASKS") # This is correct

CHECKPOINTS_DIR = os.path.join(PROCESSED_DATA_ROOT, "checkpoints_synthetic_vinifera") # <<-- MODIFIED HERE (specific to synthetic)

# Ensure all necessary directories exist
os.makedirs(COMBINED_MASKS_DIR, exist_ok=True)
os.makedirs(GEODESIC_MASKS_DIR, exist_ok=True) # Already exists from preprocessing, but good to ensure
os.makedirs(CHECKPOINTS_DIR, exist_ok=True)

# Pre-trained model path (if starting from a pre-existing model)
# Set to None if you want to train from scratch
# IMPORTANT: Update this path to your actual Algeria pre-trained model file!
PRETRAINED_MODEL_PATH = "V1_ALGERIA_best_model_vein_dice_0.7697_epoch29.pt" # <<-- IMPORTANT: UPDATE THIS!
...
NUM_WORKERS = 2 # Adjust based on your system (0 if encountering issues, but slower)
...
TRAINING_CONFIG = {
    ...
    "COMBINED_MASKS_DIR": COMBINED_MASKS_DIR, # Where combined masks are saved/loaded from
    "BLADE_MASKS_INPUT_DIR": BLADE_MASKS_INPUT_DIR, # Where individual blade masks are loaded from
    "VEIN_MASKS_INPUT_DIR": VEIN_MASKS_INPUT_DIR, # Where individual vein masks are loaded from
...
class MultiChannelLeafDataset(Dataset):
    # Added blade_masks_input_dir and vein_masks_input_dir as explicit arguments
    def __init__(self, eleven_channel_dir, blade_masks_input_dir, vein_masks_input_dir, combined_masks_output_dir, geodesic_masks_dir, transform=None):
        self.eleven_channel_dir = eleven_channel_dir
        self.blade_masks_dir = blade_masks_input_dir # <<-- MODIFIED: Renamed for clarity
        self.vein_masks_dir = vein_masks_input_dir # <<-- MODIFIED: Renamed for clarity
        self.combined_masks_output_dir = combined_masks_output_dir
        self.geodesic_masks_dir = geodesic_masks_dir
        self.transform = transform # Expects a callable (e.g., DataAugmentation instance)
        
        self.fids = sorted([f.replace(".npy", "") for f in os.listdir(eleven_channel_dir) if f.endswith(".npy")]) # Added sorted() for consistent order
    ...
    def _generate_combined_segmentation_masks(self):
        ...
        for fid in tqdm(self.fids, desc="Generating Combined Segmentation Masks"):
            # Combined mask is stored as PNG (uint8) for space efficiency and direct PIL loading
            ...
            if not os.path.exists(blade_mask_npy_path) or not os.path.exists(vein_mask_npy_path):
                print(f"WARNING: Missing blade or vein mask for {fid}. Cannot generate combined segmentation mask. Skipping.")
                # Remove this FID from self.fids so it's not attempted again
    ...
    def __getitem__(self, idx):
        ...
        # Load geodesic distance mask (H, W)
...
def dice_coefficient(predictions, targets, num_classes, ignore_index=None):
    ...
    for c in range(num_classes):
        ...
        if union == 0:
            # If both target and prediction are empty for this class, it's a perfect match (Dice=1.0)
            if target_mask.sum() == 0 and pred_mask.sum() == 0:
                dice_scores.append(1.0)
            else:
                # If target is empty but prediction is not, or vice versa, Dice=0.0
...
def geodesic_mae(pred_geodesic, target_geodesic, vein_target_mask):
    ...
    # Apply mask: only consider pixels where vein_target_mask is True
...
def train_model(pretrained_model_path=None):
    ...
    full_dataset = MultiChannelLeafDataset(
        eleven_channel_dir=ELEVEN_CHANNEL_DIR,
        blade_masks_input_dir=BLADE_MASKS_INPUT_DIR, # <<-- MODIFIED ARG
        vein_masks_input_dir=VEIN_MASKS_INPUT_DIR, # <<-- MODIFIED ARG
    ...
    if pretrained_model_path and os.path.exists(pretrained_model_path):
        ...
        # Filter out incompatible keys if the pretrained model had a different output head
        # For example, if it was only segmentation, it won't have 'geo_out_conv'
        # We assume the encoder/decoder parts are compatible.
        model_dict = model.state_dict()
        pretrained_dict = {k: v for k, v in state_dict.items() if k in model_dict and model_dict[k].shape == v.shape}
        model_dict.update(pretrained_dict)
        model.load_state_dict(model_dict) # strict=False is not needed with this approach
        print(f"Successfully loaded {len(pretrained_dict)}/{len(state_dict)} keys from pre-trained model.")
        print("Remaining layers (e.g., new geodesic head) will be randomly initialized.")
    ...
    print("\n--- Starting Synthetic Vinifera Multi-task Training ---") # <<-- MODIFIED HERE
...
if __name__ == "__main__":
    # Ensure combined mask output directory exists. The dataset class will populate it.
    os.makedirs(COMBINED_MASKS_DIR, exist_ok=True)
    # The other input directories (BLADE_MASKS, VEIN_MASKS, GEODESIC_MASKS, 11_CHANNEL_INPUTS)
    # are assumed to be populated by 3_generate_11channel_inputs.py
    ...
    # Plot Geodesic Loss
    plt.subplot(2, 3, 3)
    plt.plot(history['train_geo_loss'], label='Train Geo Loss')
    plt.plot(history['val_geo_loss'], label='Val Geo Loss')
    plt.title('Geodesic Loss per Epoch')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    plt.grid(True)
    ...
    plt.subplot(2, 3, 4)
    plt.plot(history['train_dice_background'], label='Train Dice (Background)')
    plt.plot(history['val_dice_background'], label='Val Dice (Background)', linestyle='--')
    plt.plot(history['train_dice_blade'], label='Train Dice (Blade)')
    plt.plot(history['val_dice_blade'], label='Val Dice (Blade)', linestyle='--')
    plt.plot(history['train_dice_vein'], label='Train Dice (Vein)')
    plt.plot(history['val_dice_vein'], label='Val Dice (Vein)', linestyle='--')
    plt.title('Dice Coefficient per Epoch (All Classes)')
    plt.xlabel('Epoch')
    ...
    plt.plot(history['val_geodesic_mae'], label='Val Geodesic MAE', linestyle='--')
    plt.title('Geodesic MAE per Epoch (Veins)')
    plt.xlabel('Epoch')
    plt.ylabel('MAE')
    plt.legend()
    plt.grid(True)
