...
# --- Visualization Functions (updated for radial_ects and oriented_rgb) ---
...
def create_combined_viz_from_images(ect_image_path: Path, save_path: Path,
                                     blade_color=(255, 255, 255), blade_alpha=0.2, # Reduced alpha here
...
def process_raw_leaf_shapes(raw_input_dir: Path, output_base_dir: Path, landmark_data_file: Path, clear_existing_data: bool = True):
    ...
    for i, leaf_id in enumerate(leaves_to_process_ids):
        ...
        # Paths for the different outputs
        mask_path = SHAPE_MASK_DIR / output_image_name
        ect_grayscale_path = RADIAL_ECT_DIR / output_image_name # This is the CNN input
        ect_inferno_temp_path = PROCESSED_DATA_OUTPUT_DIR / "temp_ect_inferno" / output_image_name # Temp for combined viz
        ect_inferno_temp_path.parent.mkdir(parents=True, exist_ok=True) # Ensure temp dir exists
        ...
        try:
            ...
            if rgb_image_path and rgb_image_path.exists():
                ...
                if rgb_img_cv is not None:
                    ...
                    # Create a mask in the oriented image's coordinate system
                    ...
                    # Convert oriented_rgb_img_cv to a writeable format
                    oriented_rgb_img_final = oriented_rgb_img_cv.copy() 
                    
                    # Get plasma colormap
                    plasma_cmap = cm.get_cmap('plasma')

                    # Iterate through pixels for custom coloring
                    for y in range(IMAGE_SIZE[1]):
                        for x in range(IMAGE_SIZE[0]):
                            ...
                            if mask_val == MASK_BACKGROUND_GRAY:
                                ...
                                # Get current RGB value, convert to grayscale, apply colormap
                                b, g, r = oriented_rgb_img_final[y, x]
                                grayscale_intensity = 0.2989 * r + 0.5870 * g + 0.1140 * b # Luminosity method
                                # Normalize intensity to [0, 1] for colormap
                                normalized_intensity = grayscale_intensity / 255.0
                                
                                # Get RGBA from colormap, convert to 0-255 RGB
                    ...
                    # --- Add landmarks to oriented_rgb_img_final ---
                    if transformed_landmark_points is not None and transformed_landmark_points.size > 0:
                        landmark_pixel_coords = ect_coords_to_pixels(transformed_landmark_points, IMAGE_SIZE, BOUND_RADIUS)
                        landmark_color_bgr = (0, 255, 255) # Yellow in BGR
                        landmark_size = 5
                        for lx, ly in landmark_pixel_coords:
                            if 0 <= lx < IMAGE_SIZE[0] and 0 <= ly < IMAGE_SIZE[1]:
                                cv2.circle(oriented_rgb_img_final, (lx, ly), landmark_size // 2, landmark_color_bgr, -1) # -1 for filled circle

                    cv2.imwrite(str(oriented_rgb_path), oriented_rgb_img_final)
                else:
            ...
            try:
                ...
                # Save ECT for CNN (grayscale)
                save_radial_ect_image(ect_result, ect_grayscale_path, cmap_name="gray")
                
                # Save ECT for combined visualization (inferno) in a temporary location
                save_radial_ect_image(ect_result, ect_inferno_temp_path, cmap_name="inferno")

                create_combined_viz_from_images(ect_inferno_temp_path, viz_path, # Use the inferno path here
                                                blade_color=(255, 255, 255),
                                                blade_alpha=0.2, # UPDATED ALPHA
                ...
                metadata_records.append({
                    ...
                    "file_radial_ect": str(ect_grayscale_path.relative_to(output_base_dir)) if ect_grayscale_path.exists() else "",
            ...
            metadata_records.append({
                ...
                "file_radial_ect": str(ect_grayscale_path.relative_to(output_base_dir)) if isinstance(ect_grayscale_path, Path) else "", # Metadata points to grayscale
            ...
            metadata_records.append({
                ...
                "landmark_tip_x": np.nan,
                "landmark_tip_y": np.nan
            })
            continue 
    
    # Clean up temporary inferno ECT directory
    temp_inferno_dir = PROCESSED_DATA_OUTPUT_DIR / "temp_ect_inferno"
    if temp_inferno_dir.exists():
        shutil.rmtree(temp_inferno_dir)
        print(f"Cleaned up temporary directory: {temp_inferno_dir}")
