...
# --- CONFIGURATION ---
BASE_DIR = os.getcwd() # Assumes script is run from the directory containing synthetic_dataset/ and non_synthetic_dataset/

# Input directories for your raw (source) data
SYNTHETIC_DATA_SOURCE_DIR = os.path.join(BASE_DIR, "synthetic_dataset")
NON_SYNTHETIC_DATA_SOURCE_DIR = os.path.join(BASE_DIR, "non_synthetic_dataset")

# *** NEW: Single Root output directory for ALL processed data ***
PROCESSED_OUTPUT_ROOT_DIR = os.path.join(BASE_DIR, "processed_data")

# --- Image and Mask Processing Parameters (Global for both datasets) ---
TARGET_WIDTH = 2048
TARGET_HEIGHT = 2040
...
# === HELPER FUNCTIONS (No changes here, they are shared) ===
...
def create_mask_from_coords(coords, target_size):
    ...
    draw.polygon([tuple(map(int, map(round, p))) for p in coords], fill=1)
...
def calculate_geodesic_distance_map(vein_mask_binary, geodesic_origin_xy, target_size):
    ...
    origin_x_padded, origin_y_padded = int(round(geodesic_origin_xy[0])), int(round(geodesic_origin_xy[1]))
    ...
    # Check if there are any vein pixels to prevent division by zero for max_dist
    if np.any(vein_mask_binary == 1):
        ...
        if max_dist > 0:
            ...
            # If max_dist is 0 (e.g., a single pixel vein or no traversable vein), ensure map is all zeros
...
# --- Consolidated Processing Function for Any Dataset ---
def process_image_dataset(input_source_dir, common_output_root_dir, dataset_name_prefix="synthetic"):
    """
    Processes images from a specified input directory and generates 11-channel inputs and masks,
    saving them into a common output root directory.
    
    Args:
        input_source_dir (str): The root directory containing image subfolders (e.g., synthetic_00000, non_synthetic_00000).
        common_output_root_dir (str): The single root directory where ALL processed outputs will be saved.
        dataset_name_prefix (str): A prefix for logging (e.g., "synthetic", "non_synthetic").
    """
    print(f"\n--- Processing {dataset_name_prefix.capitalize()} Data from '{input_source_dir}' ---")
    
    # Create common output directories if they don't exist
    eleven_channel_output_dir = os.path.join(common_output_root_dir, "11_CHANNEL_INPUTS")
    blade_masks_output_dir = os.path.join(common_output_root_dir, "BLADE_MASKS")
    vein_masks_output_dir = os.path.join(common_output_root_dir, "VEIN_MASKS")
    geodesic_masks_output_dir = os.path.join(common_output_root_dir, "GEODESIC_MASKS")
    overlay_check_output_dir = os.path.join(common_output_root_dir, "OVERLAY_CHECKS")
    config_output_dir = os.path.join(common_output_root_dir, "config") # Configs for this specific run

    for d in [eleven_channel_output_dir, blade_masks_output_dir, vein_masks_output_dir,
              geodesic_masks_output_dir, overlay_check_output_dir, config_output_dir]:
        os.makedirs(d, exist_ok=True)

    # Save preprocessing configuration for this dataset's run
    preprocessing_config = {
        ...
        f"SOURCE_DATA_DIR": input_source_dir,
        "COMMON_OUTPUT_ROOT_DIR": common_output_root_dir # Indicate the unified output root
    }
    # Save a config file specific to how this dataset was processed *into* the common folder
    config_file_path = os.path.join(config_output_dir, f"preprocessing_config_for_{dataset_name_prefix}.json")
    with open(config_file_path, 'w') as f:
        json.dump(preprocessing_config, f, indent=4)
    print(f"Preprocessing configuration for {dataset_name_prefix} saved to: {config_file_path}")

    # List all image subdirectories (e.g., synthetic_00000, non_synthetic_00000)
    image_fids = sorted([
        d for d in os.listdir(input_source_dir)
        if os.path.isdir(os.path.join(input_source_dir, d)) and d.startswith(f"{dataset_name_prefix}_")
    ])

    if not image_fids:
        print(f"⚠️ No '{dataset_name_prefix}_' prefixed folders found in '{input_source_dir}'. Skipping this dataset.")
        return

    for fid in tqdm(image_fids, desc=f"Processing {dataset_name_prefix.capitalize()} Images"):
        image_rgb_path = os.path.join(input_source_dir, fid, f"{fid}_rgb.png")
        geodesic_origins_json_path = os.path.join(input_source_dir, fid, f"{fid}_geodesic_origins.json")

        # Output paths now point to the common root
        ...
        # Skip if all outputs already exist for this specific FID
        ...
        try:
            ...
            if not os.path.exists(image_rgb_path):
                print(f"⚠️ Skipping {fid}: RGB image not found at '{image_rgb_path}'.")
                continue
            img_pil = Image.open(image_rgb_path).convert("RGB")
            
            # Verify target size (should already be correct from prior steps)
            if img_pil.size != TARGET_SIZE:
                print(f"  WARNING: Image {fid} has unexpected size {img_pil.size}. Resizing to {TARGET_SIZE}.")
            ...
            leaf_indices = set()
            for f in os.listdir(os.path.join(input_source_dir, fid)):
            ...
            for leaf_idx in leaf_indices:
                blade_coords_path = os.path.join(input_source_dir, fid, f"{fid}_blade_{leaf_idx}.txt")
                vein_coords_path = os.path.join(input_source_dir, fid, f"{fid}_vein_{leaf_idx}.txt")

                if os.path.exists(blade_coords_path) or os.path.exists(vein_coords_path):
            ...
            if not found_traces_for_fid:
                print(f"⚠️ Skipping mask generation for {fid}: No blade/vein coordinate files found for any leaf in '{os.path.join(input_source_dir, fid)}'.")
                continue
            ...
            print(f"❌ Error processing {fid} from {dataset_name_prefix} data: {e}. Skipping this image.")

# === MAIN EXECUTION ===
if __name__ == "__main__":
    print(f"--- Starting Data Preprocessing for All Datasets into Unified Output ---")
    
    # Ensure the single common output root directory exists
    os.makedirs(PROCESSED_OUTPUT_ROOT_DIR, exist_ok=True)

    # Process Synthetic Data, outputting to the common root
    process_image_dataset(SYNTHETIC_DATA_SOURCE_DIR, PROCESSED_OUTPUT_ROOT_DIR, "synthetic")

    # Process Non-Synthetic Data, also outputting to the common root
    process_image_dataset(NON_SYNTHETIC_DATA_SOURCE_DIR, PROCESSED_OUTPUT_ROOT_DIR, "non_synthetic")

    print(f"\n--- All Data Preprocessing Complete ---")
    print(f"All 11-channel inputs and masks are now unified in: '{PROCESSED_OUTPUT_ROOT_DIR}'!")